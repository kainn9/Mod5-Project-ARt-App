...
export const signupUser = (userData, setErrorMessage, setLoginFailed, history) => function (dispatch) {
...
export const loginUser = (userData, setErrorMessage, setLoginFailed, history) => function (dispatch) {
  const fetchConfig = {
    method: 'POST',
    headers: {
      'content-type': 'application/json',
...
export const updateUserLikes = (newLikeID) => (function (dispatch) {
  dispatch({ type: 'updateLikes', payload: newLikeID });
});
// passes in id of newly followed/unfollowed user to update action/reducer
export const updateFollows = (followID) => (function (dispatch) {
  dispatch({ type: 'updateFollows', payload: followID });
});

export const updatePosts = (postID) => (function (dispatch) {
