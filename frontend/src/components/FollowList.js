...
const FollowList = (props) => {
    ...
    const renderCardsFromUserData = (user) => {
        ...
        return user[props.relationship].map( nestedUser => 
            (  
    ...
    const fetchUser = () => {
        ...
        .then(user => {
            ...
            props.setPreviewedUserData({username: user.user.username, img: user.user.proPic})
        })
    }  
    ...
    // refetch when the logged user follower/followed relationships change and also when the previewedUserID changes(from props.userID) this is important
